...
class Data
{
    ...
    public function __construct(\Storefront\BTCPay\Model\BTCPay\BTCPayService $BTCPayService, \Magento\Framework\App\CacheInterface $cache, LoggerInterface $logger)
    {
    ...
    public function getInstallationErrors(int $magentoStoreId, bool $useCache): array
    {
        ...
        if ($errors === false) {
            ...
            if (count($specificStores) === 0) {
                ...
                $neededPermissions = $essentialPermissions;
            } else {
            ...
            if ($useCache) {
                $this->cache->save(\json_encode($errors), $cacheKey, [Config::CACHE_TAG], 15 * 60);
            }
        }
